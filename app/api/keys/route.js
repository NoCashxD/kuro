...
async function getKeys(req) {
  try {
    ...
    if (user.level === 0) {
      // Main can see all keys
      sql = 'SELECT * FROM keys_code ORDER BY created_at DESC';
      params = [];
    } else if (user.level === 1) {
      // Owner can see all keys from their admins and resellers
      sql = 'SELECT * FROM keys_code WHERE owner = ? ORDER BY created_at DESC';
      params = [user.username];
    } else if (user.level === 2) {
      // Admin can see keys from their owner's hierarchy (generated by resellers under their owner)
      sql = 'SELECT * FROM keys_code WHERE owner = ? ORDER BY created_at DESC';
      params = [user.owner];
    } else {
